...
try:
    ...
    print("Firebase Admin SDK not available. Notes functionality will be limited.")
...
async def get_user_notes(request: Request, current_user: dict = Depends(require_auth)):
    ...
    try:
        if not firestore_available:
            raise HTTPException(status_code=503, detail="Notes service is not available")
        
        db = firestore.client()
        user_id = current_user['uid']
        
        # Query notes collection for this user
        notes_ref = db.collection('notes').where('user_id', '==', user_id).order_by('created_at', direction=firestore.Query.DESCENDING)
        ...
        for note in notes:
            ...
            notes_list.append({
                'note_id': note.id,
                'title': note_data.get('title', 'Untitled Notes'),
                'created_at': note_data.get('created_at'),
                'updated_at': note_data.get('updated_at'),
                'source_type': note_data.get('source_type', 'unknown'),
                'preview': note_data.get('notes_data', {}).get('overview', '')[:200] + '...' if note_data.get('notes_data', {}).get('overview') else ''
            })
...
async def save_notes(request: Request, note_request: NoteSaveRequest, current_user: dict = Depends(require_auth)):
    ...
    try:
        ...
        note_data = {
            ...
            'encrypted': note_request.encrypt,
        ...
        logger.info(f"Notes saved successfully for user {user_id} with ID {note_id}")
...
async def get_note(note_id: str, request: Request, current_user: dict = Depends(require_auth)):
    ...
    try:
        ...
        return {
            ...
            "notes_data": note_data.get('notes_data'),
            "created_at": note_data.get('created_at'),
            "updated_at": note_data.get('updated_at'),
...
async def update_note(note_id: str, request: Request, note_update: NoteUpdateRequest, current_user: dict = Depends(require_auth)):
    ...
    try:
        ...
        if note_update.notes_data is not None:
            update_data['notes_data'] = note_update.notes_data
            update_data['encrypted'] = note_update.encrypt
