...
async def upload_video(request: Request, file: UploadFile = File(...), current_user: dict = Depends(require_auth)):
    ...
    try:
        ...
        deduction_result = await credit_manager.deduct_credits(user_id)
        if not deduction_result.get('success'):
            raise HTTPException(status_code=400, detail=f"Failed to deduct credits: {deduction_result.get('error')}")
        
        logger.info(f"Credits deducted for user {user_id}: {deduction_result.get('credits_deducted')} credits. New balance: {deduction_result.get('new_balance')}")
        ...
        try:
            ...
            # If video processing fails, refund the credits
            logger.error(f"Video processing failed for user {user_id}: {str(processing_error)}")
            refund_result = await credit_manager.refund_credits(user_id)
            if refund_result.get('success'):
                logger.info(f"Credits refunded for user {user_id}: {refund_result.get('credits_refunded')} credits")
            else:
                logger.error(f"Failed to refund credits for user {user_id}: {refund_result.get('error')}")
...
async def process_youtube(request: Request, youtube: YouTubeURL, current_user: dict = Depends(require_auth)):
    ...
    try:
        ...
        output_path = "videos/youtube_video.%(ext)s"
        logger.info(f"User {user_id} downloading YouTube video: {youtube.url}")
        
        try:
            # Use better yt-dlp options with anti-throttling measures
            cmd = [
                "yt-dlp",
            ...
            import glob
            downloaded_files = glob.glob("videos/youtube_video.*")
...
# Phi3 Ollama Testing Endpoints

class Phi3TestRequest(BaseModel):
    text: str
    use_summarization: bool = True

@app.post("/test/phi3")
async def test_phi3_ollama(request: Phi3TestRequest):
    """Test phi3 model via Ollama for text generation and summarization"""
    try:
        from utils.ollama_client import sync_generate_text, sync_chat_completion
        from utils.summarizer import get_phi3_summarizer
        ...
        logger.info("Testing phi3 text generation...")
        ...
        logger.info("Testing phi3 chat completion...")
        ...
        if request.use_summarization and len(request.text) > 100:
            logger.info("Testing phi3 educational summarization...")
        ...
        logger.error(f"Error testing phi3 Ollama: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Phi3 test failed: {str(e)}")

@app.get("/test/phi3/status")
async def get_phi3_status():
    """Get the status of phi3 integration"""
    try:
        from utils.ollama_client import sync_generate_text, sync_chat_completion
        from utils.summarizer import get_phi3_summarizer
        ...
        ollama_working = bool(test_response)
        ...
        return {
            "ollama_connection": ollama_working,
            ...
            "summarizer_status": {
                "use_ollama": summarizer.use_ollama,
                "use_fallback": summarizer.use_fallback,
                "ollama_model": summarizer.ollama_model
            },
            "recommendation": "✅ Ready for production" if ollama_working and chat_working and summarizer.use_ollama else "⚠️ Using fallback methods"
        }
        
    except Exception as e:
        import traceback
        logger.error(f"Error checking phi3 status: {str(e)}")
        logger.error(f"Traceback: {traceback.format_exc()}")
        return {
            "ollama_connection": False,
            "error": str(e),
            "traceback": traceback.format_exc(),
            "recommendation": "❌ Phi3 not available, check Ollama server"
        }
