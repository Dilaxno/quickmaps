...
async def upload_video(request: Request, file: UploadFile = File(...), current_user: dict = Depends(require_auth)):
    ...
    try:
        ...
        # Automatically save the generated notes to R2 storage
        try:
            r2_storage = get_r2_storage()
            save_result = await r2_storage.save_notes(
                user_id=user_id,
                notes_data=processing_result,
                title="",  # Let backend generate default title
                encrypt=True
            )
            logger.info(f"Educational notes auto-saved successfully for user {user_id} with ID {save_result['note_id']}")
            
            # Add save information to the result
            processing_result['auto_saved'] = True
            processing_result['note_id'] = save_result['note_id']
            processing_result['note_title'] = save_result['metadata']['title']
            
        except Exception as save_error:
            logger.error(f"Failed to auto-save notes for user {user_id}: {save_error}")
