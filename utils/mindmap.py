...
def generate_mindmap(data):
    ...
    try:
        ...
        # Add content_structure for NotesViewer compatibility
        mindmap["content_structure"] = _convert_mindmap_to_notes_structure(mindmap["data"]["children"], data)
...
def _convert_mindmap_to_notes_structure(mindmap_children, original_data):
    """Convert mindmap structure to content_structure format for NotesViewer"""
    content_structure = []
    
    for child in mindmap_children:
        # Skip overview/summary nodes
        if child.get("id") in ["overview", "summary"]:
            continue
            
        # Extract main topic - remove common emoji prefixes
        main_topic = child.get("topic", "")
        for emoji in ["🎯 ", "📋 ", "🔑 ", "📚 ", "💡 ", "🧠 "]:
            main_topic = main_topic.replace(emoji, "")
        ...
        # Handle flat structure (direct children without nested subtopics)
        if "children" in child:
            # Check if this is a flat structure (like Key Points)
            has_nested_children = any("children" in subchild for subchild in child["children"])
            
            if not has_nested_children:
                # Flat structure - create a single subtopic
                subtopic = {
                    "name": main_topic,
                    "details": [],
                    "examples": [],
                ...
                for item_node in child["children"]:
                    item_text = item_node.get("topic", "")
                    
                    # Categorize based on emoji or content
                    if "🧠" in item_text or "concept" in item_node.get("id", ""):
                        subtopic["key_points"].append(item_text.replace("🧠 ", ""))
                    elif "💡" in item_text or "example" in item_node.get("id", ""):
                        subtopic["examples"].append(item_text.replace("💡 ", ""))
                    else:
                        subtopic["details"].append(item_text)
                
                if subtopic["details"] or subtopic["examples"] or subtopic["key_points"]:
                    section["subtopics"].append(subtopic)
            else:
                # Nested structure - process subtopics
                for subtopic_node in child["children"]:
                    subtopic_name = subtopic_node.get("topic", "")
                    # Remove emoji prefixes
                    for emoji in ["📖 ", "📚 ", "📝 ", "📋 ", "📊 "]:
                        subtopic_name = subtopic_name.replace(emoji, "")
                    ...
                    # Process subtopic children (details, examples, key points)
                    if "children" in subtopic_node:
                        for detail_node in subtopic_node["children"]:
                            detail_text = detail_node.get("topic", "")
                            
                            if "🔑" in detail_text or "key" in detail_node.get("id", ""):
                                # Key point
                                subtopic["key_points"].append(detail_text.replace("🔑 ", ""))
                            elif "📝" in detail_text or "detail" in detail_node.get("id", ""):
                                # Detail
                                subtopic["details"].append(detail_text.replace("📝 ", ""))
                            elif "💡" in detail_text or "example" in detail_node.get("id", ""):
                                # Example
                                subtopic["examples"].append(detail_text.replace("💡 ", ""))
                            else:
                                # Default to detail
                                subtopic["details"].append(detail_text)
                    else:
                        # No children, use the subtopic itself as content
                        subtopic["details"].append(subtopic_name)
                    
                    # Only add subtopic if it has content
                    if subtopic["details"] or subtopic["examples"] or subtopic["key_points"]:
                        section["subtopics"].append(subtopic)
        
        # Only add section if it has subtopics
        if section["subtopics"]:
            content_structure.append(section)
    
    # If no content structure was created, create a simple one from the original data
    if not content_structure and original_data:
        fallback_section = {
            "main_topic": original_data.get("title", "Study Notes"),
            "subtopics": []
        }
        
        # Add key points if available
        if original_data.get("key_points"):
            key_points_subtopic = {
                "name": "Key Points",
                "details": [],
                "examples": [],
                "key_points": original_data["key_points"]
            }
            fallback_section["subtopics"].append(key_points_subtopic)
        
        # Add definitions if available
        if original_data.get("definitions"):
            definitions_subtopic = {
                "name": "Definitions",
                "details": [f"{d.get('term', '')}: {d.get('definition', '')}" for d in original_data["definitions"]],
                "examples": [],
                "key_points": []
            }
            fallback_section["subtopics"].append(definitions_subtopic)
        
        # Add examples if available
        if original_data.get("examples"):
            examples_subtopic = {
                "name": "Examples",
                "details": [],
                "examples": original_data["examples"],
                "key_points": []
            }
            fallback_section["subtopics"].append(examples_subtopic)
        
        if fallback_section["subtopics"]:
            content_structure.append(fallback_section)
