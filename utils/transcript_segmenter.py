...
def create_structured_prompt(sections: List[Dict]) -> str:
    ...
    # Limit sections to prevent token overflow (max 30 sections for 8B model's 6000 token limit)
    max_sections = min(30, len(sections))
    ...
    for section in selected_sections:
        ...
        prompt_parts.append(f"Key Concepts: {', '.join(section['key_concepts'][:3])}")
        prompt_parts.append(f"Content: {section['text'][:200]}...")  # Reduced to 200 chars for 8B model
    
    if len(sections) > max_sections:
        prompt_parts.append(f"\n[Note: Showing first {max_sections} of {len(sections)} sections for token limit]")
