...
def create_structured_prompt(sections: List[Dict]) -> str:
    ...
    # Limit sections to prevent token overflow (max 50 sections for large content)
    max_sections = min(50, len(sections))
    ...
    for section in selected_sections:
        ...
        prompt_parts.append(f"Content: {section['text'][:300]}...")  # Reduced from 500 to 300 chars
    
    if len(sections) > max_sections:
        prompt_parts.append(f"\n[Note: Showing first {max_sections} of {len(sections)} sections for analysis]")
