...
def sync_generate_text(
    ...
    **kwargs
    ...
    try:
        ...
        # Model configurations with their max token limits (conservative)
        model_configs = [
            ("llama-3.1-8b-instant", 4000),     # Primary: LLaMA 3.1 8B (conservative for rate limits)
            ("llama3-8b-8192", 4000),           # Fallback 1: LLaMA 3 8B (conservative)
            ("llama3-70b-8192", 4000),          # Fallback 2: LLaMA 3 70B (conservative)
            ("gemma2-9b-it", 4000)              # Fallback 3: Gemma 2 9B (conservative)
        ]
