...
class GroqClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        # Primary model and fallbacks (in order of preference for complex tasks)
        self.models = [
            "llama-3.3-70b-versatile", # Primary: LLaMA 3.3 70B (newest, best for complex JSON)
            "llama3-70b-8192",         # Fallback 1: LLaMA 3 70B (8k context, still very capable)
            "llama-3.1-8b-instant",    # Fallback 2: LLaMA 3.1 8B (131k context, fast)
...
def sync_generate_text(
    ...
    **kwargs
    ...
    try:
        ...
        # Model configurations with their max token limits (prioritizing 70B models for complex tasks)
        model_configs = [
            ("llama-3.3-70b-versatile", 8000),  # Primary: LLaMA 3.3 70B (newest, best for complex JSON)
            ("llama3-70b-8192", 8000),          # Fallback 1: LLaMA 3 70B (still very capable)
            ("llama-3.1-8b-instant", 4000),     # Fallback 2: LLaMA 3.1 8B (fast)
            ("gemma2-9b-it", 4000)              # Fallback 3: Gemma 2 9B (alternative)
        ]
...
def sync_chat_completion(
    ...
    **kwargs
    ...
    try:
        ...
        # Fallback models (prioritizing 70B models for complex tasks)
        models = [
            "llama-3.3-70b-versatile", # Primary: LLaMA 3.3 70B (newest, best for complex JSON)
            "llama3-70b-8192",         # Fallback 1: LLaMA 3 70B (still very capable)
            "llama-3.1-8b-instant",    # Fallback 2: LLaMA 3.1 8B (fast)
