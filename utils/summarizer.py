...
class EducationalSummarizer:
    ...
    def __init__(self):
        ...
        self.max_tokens = 8000  # Good balance for comprehensive notes
        self.temperature = 0.7  # Balanced for comprehensive content generation
        self.max_input_tokens = 5000  # Conservative limit to stay within Groq rate limits (6000 TPM)
    ...
    def summarize(self, segments: List[Dict]) -> Dict:
        ...
        # Check if we need to chunk the transcript due to Groq rate limits (6000 TPM)
        if total_tokens > self.max_input_tokens:
            logger.info(f"🔄 Transcript too large ({total_tokens:,} tokens), chunking into smaller parts")
    ...
    def _process_chunked_transcript(self, transcript: str, total_tokens: int) -> Dict:
        ...
        chunk_size = self.max_input_tokens - 500  # Leave room for prompt
    ...
    def _transform_to_educational_structure(self, parsed_data: Dict) -> Dict:
        ...
        try:
            ...
            educational_content = {
                ...
                "key_concepts": parsed_data.get("key_points", []),
                "detailed_notes": parsed_data.get("definitions", []),
                "examples_and_applications": parsed_data.get("detailed_examples", []),
                "study_questions": []
            }
