...
class EducationalSummarizer:
    ...
    from video transcripts using Llama 3.3 70B versatile model
    """
    
    def __init__(self):
        """Initialize the educational summarizer with powerful 70B models"""
        self.groq_model = "llama-3.3-70b-versatile"  # Latest 70B model, best for complex JSON
        self.max_tokens = 8000  # Higher limit for 70B models (can handle complex JSON)
        self.temperature = 0.7  # Balanced for comprehensive content generation
        self.max_input_tokens = 6000  # Higher limit for 70B models
    ...
    def _generate_comprehensive_notes(self, transcript: str, chunk_number: int = None, total_chunks: int = None) -> Dict:
        ...
        """
        try:
            logger.info("🧠 Generating comprehensive educational notes with Llama 3.3 70B versatile...")
...
"""
        ...
        return f"""You are an expert educational content creator with advanced knowledge in pedagogy and instructional design. Your task is to create COMPREHENSIVE, DETAILED study notes from this video transcript content.
...
- Cover ALL transcript content systematically
- Follow the JSON structure EXACTLY as specified

TRANSCRIPT CONTENT TO ANALYZE:
{transcript}

You MUST provide a detailed JSON response with this EXACT structure (fill ALL fields with comprehensive content):
...
3. Fill EVERY field in the JSON structure with detailed, comprehensive content
4. Generate AT LEAST 25+ key points, 20+ definitions, 15+ examples
5. Make sure ALL content is educational, detailed, and covers the entire transcript
6. Use Llama 3.3 70B's advanced reasoning to create truly comprehensive educational content
7. Ensure perfect JSON syntax - no trailing commas, proper escaping, valid structure

RESPOND WITH COMPREHENSIVE, DETAILED JSON NOW:"""
