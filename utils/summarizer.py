...
Please provide an extremely detailed JSON response with this ultra-comprehensive structure that explains every concept in depth with interactive elements:
{{
...
CRITICAL INSTRUCTION: Focus on creating ultra-comprehensive, research-grade educational materials that explain every single word, phrase, and concept in exhaustive detail. Provide multiple levels of explanation from elementary to expert level. Students and researchers should be able to understand every nuance, implication, and connection. Consider different learning styles, cognitive loads, and provide scaffolding for complex concepts. Ensure content is academically rigorous while providing pathways for all levels of learners. Every technical term should be explained as if the reader has never encountered it before, while also providing expert-level insights. Respond only with valid JSON."""
    ...
    def _create_structured_summary_fallback(self, transcript: str) -> Dict:
        ...
        # Deep analysis features with interactive elements for comprehensive understanding
        word_by_word_analysis = []
        for i, sentence in enumerate(sentences[:5]):  # Analyze first 5 sentences in detail
            if len(sentence) > 20:
                words = sentence.split()
                word_breakdown = []
                for j, word in enumerate(words[:10]):  # Analyze first 10 words of each sentence
                    if len(word) > 3:  # Skip very short words
                        clean_word = word.strip('.,!?;:')
                        word_breakdown.append({
                            "word": clean_word,
                            "word_id": f"word_{i}_{j}",
                            "meaning": f"In this context, '{clean_word}' refers to a key concept that requires detailed explanation",
                            "technical_definition": f"Academic definition: {clean_word} represents a fundamental element in this field of study",
                            "why_important": f"This word is crucial because it establishes the foundation for understanding the broader concept",
                            "related_concepts": [f"Related concept 1 for {clean_word}", f"Related concept 2 for {clean_word}"],
                            "sub_explanations": [
                                {
                                    "aspect": "etymological analysis",
                                    "explanation": f"The word '{clean_word}' has specific origins and evolution in academic discourse",
                                    "examples": [f"Historical usage of {clean_word}", f"Modern interpretation of {clean_word}"],
                                    "clickable_id": f"word_etymology_{i}_{j}"
                                },
                                {
                                    "aspect": "contextual significance",
                                    "explanation": f"In this specific context, '{clean_word}' carries particular meaning and implications",
                                    "examples": [f"Context-specific usage", f"Field-specific interpretation"],
                                    "clickable_id": f"word_context_{i}_{j}"
                                }
                            ],
                            "interactive_prompts": [
                                f"Why is '{clean_word}' the most precise word choice here?",
                                f"How does '{clean_word}' connect to broader theoretical frameworks?",
                                f"What would happen if we used a different word instead of '{clean_word}'?"
                            ]
                        })
