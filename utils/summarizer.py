...
Focus only on content from this specific segment. Respond only with valid JSON."""
            ...
            prompt = f"""Analyze this educational content and create a comprehensive learning summary in JSON format. Extract as much educational value as possible.
...
{{
  ...
  "summary": "Comprehensive summary in 3-4 sentences covering the main points",
  "key_concepts": ["concept1", "concept2", "concept3", "concept4", "concept5"],
  "definitions": {{
    "term1": "detailed definition with context",
    "term2": "detailed definition with context",
    "term3": "detailed definition with context"
  }},
  "important_facts": [
    "Important fact 1 with context",
    "Important fact 2 with context", 
    "Important fact 3 with context",
    "Important fact 4 with context"
  ],
  "examples": [
  ...
  "processes": [
    "Step-by-step process or method 1",
    "Step-by-step process or method 2"
  ],
  "applications": [
...
Extract as much educational content as possible. Be thorough and detailed. Respond only with valid JSON."""
    ...
    def _generate_summary_with_llama(self, transcript: str) -> Dict:
        ...
        try:
            ...
            if self.use_groq:
                ...
                system_prompt = """You are an expert educational content analyzer and learning specialist. Your task is to extract maximum educational value from content and create comprehensive, detailed learning summaries. 

Key requirements:
- Extract ALL key concepts, definitions, and important facts
- Identify practical applications and real-world examples
- Create meaningful learning objectives and assessment questions
    ...
    def summarize_full_transcript(self, full_transcript: str, segments: List[Dict]) -> Dict:
        ...
        try:
            if self.use_groq:
                ...
                user_prompt = f"""Analyze this educational video transcript and create a DYNAMIC mind map structure based on the actual content flow and topics discussed.

TRANSCRIPT:
{transcript_excerpt}{truncation_note}

Create a JSON response that organizes the content into NATURAL TOPICS and SUBTOPICS as they appear in the video, following this structure:
...
{{
  ...
  "summary": "Brief 2-sentence summary of the entire video"
}}

IMPORTANT: 
- Organize content by ACTUAL TOPICS discussed, not generic categories
- Follow the natural flow and progression of the video
- Create 3-6 main topics maximum
- Each main topic should have 2-4 subtopics
- Focus on the specific subject matter, not generic educational structure
        ...
        for chapter in chapters:
            ...
            try:
                if self.use_groq:
                    ...
                    truncated_note = "... [content truncated]" if len(chapter['text']) > 4000 else ""
                    
                    user_prompt = f"""Analyze this chapter from an educational video and provide a JSON summary:

CHAPTER {chapter_id} ({duration:.1f}s duration):
{chapter_text}{truncated_note}
