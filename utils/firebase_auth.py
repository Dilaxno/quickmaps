...
ALLOW_TOR_USERS = os.getenv("ALLOW_TOR_USERS", "false").lower() == "true"  # If true, allows TOR users despite VPN blocking
...
def check_ip_allowed(request: Request) -> tuple[bool, dict]:
    ...
    # Special handling for Tor users if allowed
    if should_block and ALLOW_TOR_USERS and result.get('proxy_type') == 'TOR':
        logger.info(f"Allowing TOR user from {client_ip} (ALLOW_TOR_USERS=true)")
        should_block = False
    ...
    if request:
        ...
        if not ip_allowed:
            ...
            if proxy_type == 'VPN':
                ...
                if ALLOW_TOR_USERS:
                    friendly_message = "We detected you're using Tor browser. While we support privacy-focused users, some features may be limited. If you continue to experience issues, please contact support."
                else:
